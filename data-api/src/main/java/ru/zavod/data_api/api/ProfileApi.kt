...
class ProfileApi @Inject constructor(apiProvider: ApiProvider) {
    ...
    suspend fun updateUser(params: UpdateMeParams) {
        val response = api.updateMe(updateMeDto = params.toDto())
        checkResponse(response = response)
    }
}